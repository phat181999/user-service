...
export class CacheInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<any> {
                ...
                next.handle().pipe(
                    tap((data) => {
                        if (Array.isArray(data)) {
                            this.logger.log(`Storing array of ${data.length} objects in cache for ${cacheKey}`);
                        }
        ...
        if (cachedData) {
            this.logger.log(`Cache hit for ${cacheKey}, returning ${Array.isArray(cachedData) ? cachedData.length : 1} objects`);
